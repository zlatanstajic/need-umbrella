...
(() => {
  ...
  var STRINGS = {
    sr: {
      ...
      locationTimeLabel: "Vreme na lokaciji",
    ...
    en: {
      ...
      locationTimeLabel: "Location time",
  ...
  function normalizeShowLocationTime(value) {
  ...
  function resolveTimeZone(lat, lon) {
    ...
    try {
      return (0, import_tz_lookup.default)(lat, lon);
    } catch (e) {
      return null;
    }
  }
  function formatTimeInZone(timeZone, instant, lang, browserTimeZone) {
    try {
      ...
      var dateLabel = sameDate(locationDate, browserDate) ? null : new Intl.DateTimeFormat(locale, {
        timeZone,
        year: "numeric",
        month: "short",
        day: "numeric"
  ...
  function targetCoordinatesMatch(target, lat, lon) {
    return !!target.timeZone && target.timeLat === round4(lat) && target.timeLon === round4(lon);
  }
  function resetTimeTarget(target) {
    target.timeZone = void 0;
    target.timeLat = void 0;
    target.timeLon = void 0;
    if (target.timeBox) {
  ...
  function setTargetCoordinates(target, lat, lon) {
  ...
  function renderTimeTarget(target, instant, lang) {
    if (!target.timeBox || !target.timeValue || !target.timeDate) {
      return false;
    }
  ...
  var LocationTimeUpdater = class {
    constructor(targets, lang, now) {
      this.timer = null;
    ...
    schedule() {
      var self = this;
      var delay = 1e3 - this.now().getTime() % 1e3;
  ...
  var LatestRequestCoordinator = class {
    ...
    runErrorIfCurrent(token, preserveExisting, callback) {
      if (preserveExisting) {
        return false;
      }
  ...
  // src/app.ts
  var showLocationTime = normalizeShowLocationTime(store.get("showLocationTime", void 0));
  ...
  var timeUpdater = new LocationTimeUpdater(
    function() {
      ...
      if (compareMode && secondarySlot.timeZone) {
        targets.push(secondarySlot);
      }
      return targets;
    },
    function() {
  ...
  function hideTimeTarget(slot) {
    if (slot.timeBox) {
      slot.timeBox.classList.add("hidden");
    }
  }
  function syncLocationTimeUpdater() {
  ...
  function loadWeather(loc, preserveSameCoordinates) {
    ...
    fetch(metUrl(lat, lon)).then(function(res) {
      if (!primaryRequests.isCurrent(requestToken)) {
        return null;
      }
      if (!res.ok) {
      ...
      if (!primaryRequests.isCurrent(requestToken) || !data) {
        return;
      }
  ...
  function loadSecondary(loc, preserveSameCoordinates) {
    ...
    fetch(metUrl(lat, lon)).then(function(res) {
      if (!secondaryRequests.isCurrent(requestToken)) {
        return null;
      }
      if (!res.ok) {
        throw new Error(tf("httpError", res.status));
      ...
      if (!secondaryRequests.isCurrent(requestToken) || !data) {
        return;
      }
      var timeseries = data && data.properties && data.properties.timeseries;
      if (!timeseries || !timeseries.length) {
  ...
  var locationTimeToggle = el("location-time-toggle");
  function applyLocationTimePreference(on) {
    showLocationTime = on;
